...
import { API_BASE_URL, getApiErrorMessage } from '../../services/httpClient'
...
function CommercialListPage({ kind }: { kind: DocumentKind }) {
  ...
  return <div className="crud-page">
    ...
    {loading ? <LoadingState label={`Carregando ${documentTitle(kind).toLowerCase()}...`} /> : error ? <ErrorState description={error} onRetry={load} /> : documents.length === 0 ? <div className="data-card"><EmptyState title={`Nenhum ${isQuote ? 'orçamento' : 'pedido'} encontrado`} description="Crie o primeiro documento para iniciar o fluxo comercial." /></div> : <div className="data-card data-table-wrap mobile-row-cards commercial-documents-table" {...tableCustomization}><table className="data-table"><thead><tr><th>Número</th><th>Cliente</th><th>Status</th><th>Total</th><th><span className="sr-only">Ações</span></th></tr></thead><tbody>{documents.map((document) => <tr key={document.id}><td className="data-primary" data-label="Número">{document.numero}<span className="data-secondary">#{document.id}</span><span className="data-secondary">{customerName(document.cliente_id)}</span></td><td data-label="Cliente">{customerName(document.cliente_id)}</td><td data-label="Status">{statusLabel(document.status)}</td><td data-label="Total">{money(document.total)}</td><td data-label="Ações"><div className="table-actions"><button className="table-action" type="button" onClick={() => setSelected(document)}>Ver</button>{canEdit(document) ? <button className="table-action" type="button" onClick={() => setEditing(document)}>Editar</button> : null}<a className="table-action" href={`${API_BASE_URL}${isQuote ? api.quotePrintUrl(document.id) : api.orderPrintUrl(document.id)}`} target="_blank" rel="noreferrer">Imprimir</a></div></td></tr>)}</tbody></table></div>}
...
export function ReturnsPage() {
  ...
  return <div className="crud-page"><div className="crud-page-header"><PageHeader eyebrow="Comercial" title="Devoluções" description="Registre devoluções totais ou parciais e acompanhe seu efeito." pageId="returns" /><button className="button button-primary" type="button" onClick={() => { if (saleOptions[0]) setSelectedSale(saleOptions[0]); else setError('Não há vendas disponíveis para devolução.') }}>+ Nova devolução</button></div>{feedback ? <FeedbackBanner kind="success" message={feedback} onDismiss={() => setFeedback(null)} /> : null}{error ? <FeedbackBanner kind="error" message={error} onDismiss={() => setError(null)} /> : null}<div className="data-card data-table-wrap mobile-row-cards returns-table"><table className="data-table"><thead><tr><th>Devolução</th><th>Venda</th><th>Motivo</th><th>Status</th><th>Ações</th></tr></thead><tbody>{returns.map((record) => <tr key={record.id}><td data-label="Devolução">#{record.id}</td><td data-label="Venda">#{record.venda_id}</td><td data-label="Motivo">{record.motivo}</td><td data-label="Status">{record.status}</td><td data-label="Ações">{record.status === 'RASCUNHO' ? <button className="table-action" type="button" onClick={() => void approve(record)}>Aprovar</button> : null}</td></tr>)}</tbody></table>{returns.length === 0 ? <EmptyState title="Nenhuma devolução registrada" description="As devoluções aparecerão aqui após o primeiro registro." /> : null}</div>{selectedSale ? <Modal title={`Devolução da venda #${selectedSale.id}`} onClose={() => setSelectedSale(null)}><ReturnForm sale={selectedSale} onSaved={save} onCancel={() => setSelectedSale(null)} /></Modal> : null}</div>
}
