...
function DocumentDetails({ document, kind, onStatus, onConvert }: { document: CommercialDocumentBase; kind: DocumentKind; onStatus: (status: string) => void; onConvert: () => void }) {
  const transitions = nextStatuses(kind, document.status)
  return <div>
    <dl className="detail-grid"><div><dt>Número</dt><dd>{document.numero}</dd></div><div><dt>Cliente</dt><dd>#{document.cliente_id}</dd></div><div><dt>Status</dt><dd>{statusLabel(document.status)}</dd></div><div><dt>Total</dt><dd>{money(document.total)}</dd></div><div><dt>Origem</dt><dd>{'orcamento_id' in document && document.orcamento_id ? `Orçamento #${document.orcamento_id}` : 'Sem origem'}</dd></div><div><dt>Condição de pagamento</dt><dd>{document.condicao_pagamento_id ? `#${document.condicao_pagamento_id}` : 'Não informada'}</dd></div></dl>
...
function CommercialListPage({ kind }: { kind: DocumentKind }) {
  ...
  return <div className="crud-page">
    ...
    {selected ? <Modal title={`${isQuote ? 'Orçamento' : 'Pedido'} ${selected.numero}`} size="large" onClose={() => setSelected(null)}><DocumentDetails document={selected} kind={kind} onStatus={(next) => void changeStatus(selected, next)} onConvert={() => void convert(selected)} /></Modal> : null}
  </div>
}
