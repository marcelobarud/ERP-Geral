...
function statusLabel(value: string): string { return value.replace('ABERTO', 'Aberto').replace('PARCIAL', 'Parcial').replace('PAGO', 'Pago').replace('VENCIDO', 'Vencido').replace('CANCELADO', 'Cancelado') }

function SettlementForm({ installment, accounts, onSaved, onCancel }: { installment: FinancialTitle['parcelas'][number]; accounts: FinancialAccount[]; onSaved: () => void; onCancel: () => void }) {
  const [accountId, setAccountId] = useState<number | ''>(accounts[0]?.id ?? '')
  const [value, setValue] = useState(String(Number(installment.valor) - Number(installment.valor_liquidado)))
  const [date, setDate] = useState(new Date().toISOString().slice(0, 10))
  const [observation, setObservation] = useState('')
  const [error, setError] = useState<string | null>(null)
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!accountId || !value) { setError('Informe conta e valor.'); return } try { await api.settleInstallment(installment.id, { conta_id: accountId, valor: value, data_liquidacao: date, observacao: observation || null }); onSaved() } catch (saveError) { setError(getApiErrorMessage(saveError, 'Não foi possível registrar a liquidação.')) } }
  return <form onSubmit={submit}>{error ? <FeedbackBanner kind="error" message={error} /> : null}<label className="form-field"><span>Conta</span><select required value={accountId} onChange={(event) => setAccountId(Number(event.target.value) || '')}><option value="">Selecione</option>{accounts.map((account) => <option value={account.id} key={account.id}>{account.nome}</option>)}</select></label><label className="form-field"><span>Valor</span><input required min="0.01" step="0.01" type="number" value={value} onChange={(event) => setValue(event.target.value)} /></label><label className="form-field"><span>Data</span><input required type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label className="form-field"><span>Observação</span><textarea value={observation} onChange={(event) => setObservation(event.target.value)} /></label><div className="form-actions"><button className="button button-secondary" type="button" onClick={onCancel}>Cancelar</button><button className="button button-primary" type="submit">Confirmar liquidação</button></div></form>
}

export function FinancialTitlesPage({ kind }: { kind: 'RECEBER' | 'PAGAR' }) {
  const [titles, setTitles] = useState<FinancialTitle[]>([]); const [accounts, setAccounts] = useState<FinancialAccount[]>([]); const [status, setStatus] = useState(''); const [selected, setSelected] = useState<FinancialTitle | null>(null); const [settling, setSettling] = useState<FinancialTitle['parcelas'][number] | null>(null); const [error, setError] = useState<string | null>(null); const [feedback, setFeedback] = useState<string | null>(null)
