  ...
  return <div className="crud-page">
    <div className="crud-page-header"><PageHeader eyebrow="Cadastros" title="Fornecedores" description="Mantenha os parceiros do seu negócio organizados." pageId="suppliers" /><button className="button button-primary" type="button" {...createButtonCustomization} onClick={() => { setSelected(null); setModal('create'); setFeedback(null) }}>+ Novo fornecedor</button></div>
      ...
      <section className="filter-toolbar" aria-label="Filtros de fornecedores">
        <SearchInput value={searchDraft} onChange={setSearchDraft} onSearch={(value) => setAppliedFilters((current) => { const search = value.trim(); if (current.search === search) return current; const { search: _search, ...filters } = current; return search ? { ...filters, search } : filters })} onClear={() => setSearchDraft('')} label="Pesquisar fornecedores" customizationKey="suppliers.search_input" customizationPage="suppliers" />
